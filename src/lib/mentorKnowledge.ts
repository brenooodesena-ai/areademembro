...
export const MENTOR_KNOWLEDGE: Record<string, StrategicResponse> = {
    ...
    traffic_paid: {
        ...
        definition: "Tráfego Pago não é apenas 'comprar cliques'; é a engenharia reversa de previsibilidade de escala e a injeção deliberada de capital em algoritmos de aprendizado de máquina (Machine Learning) para multiplicar receita. É o motor principal para transformar ofertas validadas em máquinas de caixa previsível.",
        strategy: "Para operar tráfego como um estrategista de elite, abandone o pensamento amador de 'subir campanha e cruzar os dedos'. \nImplemente uma estrutura baseada em quatro vetores invioláveis:\n\n1. **Fase de Inteligência (Mineração de Ângulos)**: Teste radical de ganchos (hooks) em criativos, isolando variáveis. Encontre a mensagem exata que interrompe o padrão do seu público antes sequer de se preocupar com a página de vendas.\n2. **Fase de Tração (Estabilização do Pixel)**: Foque inicialmente em gerar volume de dados qualificados. O algoritmo exige um ecossistema com conversões consistentes (idealmente 50 eventos na semana) para sair do aprendizado e otimizar o seu Custo por Aquisição (CPA).\n3. **Mecânicas de Escala**: Diferencie Escala Vertical de Horizontal. \n- *Vertical*: Aumente o orçamento (15% a 25% a cada 2-3 dias) apenas nos ativos que quebram o patamar do seu ROAS mínimo viável, respeitando o limite elástico do leilão sem resetar o algoritmo.\n- *Horizontal*: Duplique e paralelize conjuntos (novos interesses, lookalikes em 1%, 3% e 5%, broad targeting aberto) mantendo orçamentos que você já domina e pulverizando o risco.\n4. **Recuperação Cínica (Retargeting de Base)**: Sua maior margem de lucro líquido está escondida em campanhas OMNICHANNEL focadas em quebrar objeções ferozes para quem evadiu do checkout ou da landing page. \n\nO jogo não se ganha apenas olhando para as colunas do gerenciador, mas analisando o **MER (Marketing Efficiency Ratio)** — a relação entre seu faturamento global e seu investimento total diário.",
        action: "Realize uma auditoria imediata de esteira: Isole o seu criativo de maior CTR e menor CPC dos últimos 15 dias. Suba uma campanha CBO (Otimização a nível de Campanha) robusta com 3 conjuntos distintos (Público de Interesse hipersegmentado, Lookalike focado em quem comprou, e um Público totalmente Aberto), e deixe rodar por 72 horas em absoluto silêncio, sem tocar nas métricas para não sabotar o aprendizado.",
        metric: "CPA (Custo Por Aquisição Máximo) vs LTV (Lifetime Value) - Ganha a guerra no tráfego quem sabe exatamente até quanto pode sangrar para adquirir um cliente sem quebrar seu caixa no curto prazo, extraindo lucro vitalício nas vendas subsequentes."
    },
    traffic_organic: {
